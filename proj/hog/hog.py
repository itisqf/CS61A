...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        i = 1
        gcd = 1
        for i in range(1, min_score + 1):
            if min_score % i == 0 and max_score % i == 0:
                gcd = i
        if gcd >= 10:
            return True
        else:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        if who == 0:
            roll_time0 = strategy0(score0, score1)
            score0 += take_turn(roll_time0, score1, dice)
            ...
            if not extra_turn(score0, score1):
                who = other(who)
        if who == 1:
            roll_time1 = strategy1(score1, score0)
            score1 += take_turn(roll_time1, score0, dice)
            say = say(score0, score1)
            if score0 >= goal or score1 >= goal:
                break
...
def make_averaged(original_function, trials_count=1000):
    ...
    def print_and_return(*args):
        ...
        for i in range(trials_count):
            total_sum += original_function(*args)
        result = total_sum / trials_count
        return result
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num_of_rolls = 0
    for i in range (1, 11):
        cur_average = averaged_score(i, dice)
        if cur_average > max_cycle_score:
            max_cycle_score = cur_average
            num_of_rolls = i
    return num_of_rolls
    # END PROBLEM 9
